...
const SecondaryButton: FC<Props> = forwardRef(
    (props: Props, ref: Ref<HTMLButtonElement>) => {
        ...
        const className = cx(
            'px-4 bg-transparent text-teal-500 border border-teal-500 font-semibold rounded-sm inline-flex items-center justify-center transition-all duration-200 min-w-button shadow',
            {
                'h-6': props.size === 'xs',
                'h-8': props.size === 'sm',
                'h-10': props.size === 'md',
